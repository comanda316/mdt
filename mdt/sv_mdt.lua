...
AddEventHandler("mdt:getOffenderDetails", function(offender)
	...
	local convictions = MySQL.Sync.fetchAll('SELECT * FROM `user_convictions` WHERE `identifier` = @id', {
		['@id'] = offender.id
	})
	...
	local warrants = MySQL.Sync.fetchAll('SELECT * FROM `mdt_warrants` WHERE `identifier` = @id', {
		['@id'] = offender.id
	})
	if warrants[1] then
...
AddEventHandler("mdt:getOffenderDetailsById", function(identifier)
	local usource = source
...
AddEventHandler("mdt:saveOffenderChanges", function(id, changes, identifier)
	...
	MySQL.Async.fetchAll('SELECT * FROM `users` WHERE `identifier` = @id', {
		...
		if result[1] then
			MySQL.Async.execute('UPDATE `users` SET `notes` = @notes, `mugshot_url` = @mugshot_url, `bail` = @bail WHERE `identifier` = @id', {
				['@id'] = id,
				['@notes'] = changes.notes,
			...
			MySQL.Async.insert('INSERT INTO `users` (`identifier`, `notes`, `mugshot_url`, `bail`) VALUES (@id, @notes, @mugshot_url, @bail)', {
				['@id'] = id,
				['@notes'] = changes.notes,
				['@mugshot_url'] = changes.mugshot_url,
				['@bail'] = changes.bail
		...
		if changes.convictions ~= nil then
			for conviction, amount in pairs(changes.convictions) do	
				MySQL.Async.execute('UPDATE `user_convictions` SET `count` = @count WHERE `identifier` = @id AND `offense` = @offense', {
					['@id'] = id,
					['@count'] = amount,
					['@offense'] = conviction
				})
		...
		for i = 1, #changes.convictions_removed do
			MySQL.Async.execute('DELETE FROM `user_convictions` WHERE `identifier` = @id AND `offense` = @offense', {
				['@id'] = id,
...
AddEventHandler("mdt:submitNewReport", function(data)
	...
	MySQL.Async.insert('INSERT INTO `mdt_reports` (`identifier`, `title`, `incident`, `charges`, `author`, `name`, `date`) VALUES (@id, @title, @incident, @charges, @author, @name, @date)', {
		['@id']  = data.identifier,
		['@title'] = data.title,
		['@incident'] = data.incident,
		['@charges'] = charges,
	...
	for offense, count in pairs(data.charges) do
		MySQL.Async.fetchAll('SELECT * FROM `user_convictions` WHERE `offense` = @offense AND `identifier` = @id', {
			['@offense'] = offense,
			['@id'] = data.identifier
		}, function(result)
			if result[1] then
				MySQL.Async.execute('UPDATE `user_convictions` SET `count` = @count WHERE `offense` = @offense AND `identifier` = @id', {
					['@id']  = data.identifier,
					['@offense'] = offense,
					['@count'] = count + 1
				})
			else
				MySQL.Async.insert('INSERT INTO `user_convictions` (`identifier`, `offense`, `count`) VALUES (@id, @offense, @count)', {
					['@id']  = data.identifier,
					['@offense'] = offense,
					['@count'] = count
				})
...
AddEventHandler("mdt:getVehicle", function(vehicle)
	...
	local bail = MySQL.Sync.fetchAll('SELECT `bail` FROM users WHERE `identifier` = @id', {
		['@id'] = vehicle.owner_id
	})
...
AddEventHandler("mdt:submitNewWarrant", function(data)
	...
	data.date = os.date('%m-%d-%Y %H:%M:%S', os.time())
	MySQL.Async.insert('INSERT INTO `mdt_warrants` (`name`, `identifier`, `report_id`, `report_title`, `charges`, `date`, `expire`, `notes`, `author`) VALUES (@name, @identifier, @report_id, @report_title, @charges, @date, @expire, @notes, @author)', {
		['@name']  = data.name,
...
AddEventHandler("mdt:saveVehicleChanges", function(data)
	...
	MySQL.Async.fetchAll('SELECT * FROM `owned_vehicles` WHERE `plate` = @plate', {
		...
		if result[1] then
			MySQL.Async.execute('UPDATE `owned_vehicles` SET `stolen` = @stolen, `notes` = @notes WHERE `plate` = @plate', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
		else
			MySQL.Async.insert('INSERT INTO `owned_vehicles` (`plate`, `stolen`, `notes`) VALUES (@plate, @stolen, @notes)', {
				['@plate'] = data.plate,
				['@stolen'] = data.stolen,
				['@notes'] = data.notes
			})
